...
# from utils.preprocess1 import load_or_preprocess_datasets
from utils import preprocess1_timefeat as p_time
from utils import preprocess1_time_weather as p_timeweather
from utils.visualization import plot_prediction


def _resolve_household_key(requested_id: str, keys_iterable):
    """Map 'H14' <-> 'H14_Wh' etc. to the actual key in tr/va/te."""
    keys = set(keys_iterable)
    # exact
    if requested_id in keys:
        return requested_id
    base = requested_id.split('_')[0]  # 'H14_Wh' -> 'H14'
    # try removing '_Wh'
    if requested_id.endswith("_Wh"):
        cand = requested_id[:-3]
        if cand in keys:
            return cand
    # try adding '_Wh'
    if not requested_id.endswith("_Wh"):
        cand = requested_id + "_Wh"
        if cand in keys:
            return cand
    # base only
    if base in keys:
    ...
    for k in keys:
        if k.startswith(base):
            return k
    raise KeyError(f"Household id '{requested_id}' not found. First keys: {sorted(list(keys))[:10]}")
...
class LSTMClient(fl.client.NumPyClient):
    def __init__(self, cid, household_id, config, device, data_dir, save_dir):
        ...
        loader = p_timeweather.load_or_preprocess_datasets if config.get("use_weather_features", False) \
            else p_time.load_or_preprocess_datasets

        tr, va, te, _ = loader(config, data_dir)
        ...
        feat_dim = 1  # consumption
        if config.get("use_time_features", False):
            feat_dim += 2  # hour, weekday
        if config.get("use_weather_features", False):
            feat_dim += 3  # drybulb, soltot, rain
