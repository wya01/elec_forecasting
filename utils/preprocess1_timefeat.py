...
class PowerDataset(Dataset):
    ...
    def create_samples(self):
        ...
        for i in range(len(self.series) - self.window_size - self.prediction_horizon + 1):
            ...
            x = np.hstack([x_values, x_time])  # 拼接数值与时间特征
            X.append(x)
    ...
    def __getitem__(self, idx):
        return (
            torch.tensor(self.X[idx], dtype=torch.float32),  # [seq_len, channels]
...
# ==== Full preprocessing with time features ====
...
# ==== Cached version with timefeat suffix ====
def load_or_preprocess_datasets(config, data_dir, cache_dir="data/cached"):
    ...
    print("Preprocessing raw data (with time features)...")
